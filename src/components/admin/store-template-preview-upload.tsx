...
/** Vignette d'aperçu d'un template Boutique — équivalent de `TemplatePreviewUpload` (Restaurant). */
export function StoreTemplatePreviewUpload({
  templateKey,
  previewUrl,
}: {
  templateKey: string;
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      await uploadFile('/api/admin/store-template-assets', formData);
      router.refresh();
    } catch (err) {
  ...
  return (
    <div>
      {previewUrl ? (
        ...
        <img src={previewUrl} alt="" className="mb-3 h-32 w-full rounded-lg object-cover" />
      ) : (
      ...
      <input
        ...
        className="sr-only"
        id={`store-template-preview-${templateKey}`}
        onChange={(event) => {
      ...
      />
      <label
        htmlFor={`store-template-preview-${templateKey}`}
