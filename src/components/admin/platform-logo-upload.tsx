...
/**
 * Envoi du logo de la plateforme par le Super Admin.
 *
 * Clé de stockage fixe (`platform/logo.png`) : un nouvel envoi écrase
 * l'ancien à la même URL, donc `router.refresh()` suffit à le faire
 * réapparaître partout (en-têtes, tableau de bord) sans rien recâbler.
 */
export function PlatformLogoUpload({ logoUrl }: { logoUrl: string | null }) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      await uploadFile('/api/admin/platform-assets', formData);
      router.refresh();
    } catch (err) {
  ...
  return (
    <div className="flex items-center gap-4">
      {logoUrl ? (
        // eslint-disable-next-line @next/next/no-img-element -- logo de plateforme, hôte de stockage arbitraire
        <img
      ...
      <div>
        ...
        >
          {uploading ? 'Envoi…' : logoUrl ? 'Remplacer le logo' : 'Envoyer le logo'}
        </label>
