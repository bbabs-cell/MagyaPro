...
import type { BoutiqueSector } from '@/lib/storage';

/**
 * Envoi de l'image illustrant un secteur d'activité (Habillement,
 * Électronique...) sur la page d'accueil MagyaPro Boutique — une par
 * valeur de `StoreBusinessType`.
 */
export function BoutiqueSectorUpload({
  sector,
  label,
  imageUrl,
}: {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      await uploadFile('/api/admin/boutique-sector-assets', formData);
      router.refresh();
    } catch (err) {
  ...
  return (
    <div>
      ...
      <input
        ...
        className="sr-only"
        id={`boutique-sector-upload-${sector}`}
        onChange={(event) => {
      ...
      />
      <label
        htmlFor={`boutique-sector-upload-${sector}`}
