...
/**
 * Envoi de l'illustration d'une étape « Comment ça fonctionne » de la page
 * d'accueil MagyaPro Boutique — équivalent de `HowItWorksUpload` côté
 * Restaurant, sur son propre point d'entrée (`boutique-how-it-works-assets`).
 */
export function BoutiqueHowItWorksUpload({
  step,
  title,
  imageUrl,
}: {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      await uploadFile('/api/admin/boutique-how-it-works-assets', formData);
      router.refresh();
    } catch (err) {
  ...
  return (
    <div>
      ...
      <input
        ...
        className="sr-only"
        id={`boutique-how-it-works-upload-${step}`}
        onChange={(event) => {
      ...
      />
      <label
        htmlFor={`boutique-how-it-works-upload-${step}`}
